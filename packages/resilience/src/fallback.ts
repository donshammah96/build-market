  ...
  try {
    ...
    if (options.fallbackFn) {
      try {
        ...
        options.logger?.error('Fallback function also failed', fallbackError as Error);
 ...
 */
export function createFallbackWrapper<T>(
  options: FallbackOptions<T>
) {
...
export async function withGracefulDegradation<T>(
  ...
  for (let i = 0; i < operations.length; i++) {
    try {
      ...
      if (i > 0 && logger) {
        logger.info(`Succeeded with fallback strategy ${i + 1}/${operations.length}`);
      }
