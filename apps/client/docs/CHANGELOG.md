...
- **Edge Middleware Resilience, Staging Publishable Key Reconciliation & M-Pesa Webhook Error Guarding (`apps/client/middleware.ts`, `apps/client/app/api/webhooks/mpesa/stk-callback/route.ts`)**:
  ...
  - Guarded `addMpesaStkCallbackJob` in `apps/client/app/api/webhooks/mpesa/stk-callback/route.ts` with try/catch returning `acceptedResponse(503)` instead of throwing an unhandled HTTP 500 exception on transient Redis/BullMQ errors.
