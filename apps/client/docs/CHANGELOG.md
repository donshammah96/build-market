...
- **Edge Webhook Route Classification & Staging Non-Satellite Clerk Domain Sanitization (`apps/client/app/lib/security/middleware/route-matcher.ts`, `apps/client/middleware.ts`, `apps/client/app/layout.tsx`, `apps/client/app/lib/infrastructure/env.ts`, `apps/client/__tests__/middleware/middleware.test.ts`, `apps/client/__tests__/middleware/route-matrix.test.ts`, `apps/client/__tests__/middleware/clerk-middleware-dispatch.test.ts`)**:
  - Added `"/api/webhooks/(.*)"` wildcard coverage to `PUBLIC_API_ROUTES` and `isSettingsExemptRoute` in `route-matcher.ts`, allowing inbound provider webhooks (`/api/webhooks/mpesa/*`, etc.) to pass edge middleware to execute their dedicated signature and token authenticity checks (`verifyMpesaCallbackAuthenticity`) rather than being denied at the edge with `mw_deny_api_unclassified` (401 `{"error":"Not found"}`). Resolves Cypress `AssertionError: expected [ 200, 202 ] to include 401` in `04-mpesa-replay-and-idempotency.cy.ts`.
  ...
  - Enforced the non-satellite primary app invariant in `apps/client/app/layout.tsx` and `apps/client/app/lib/infrastructure/env.ts`: when `isSatellite` is `false`, any inherited `NEXT_PUBLIC_CLERK_DOMAIN` is stripped from server `process.env` so `ClerkProvider` does not emit `data-clerk-domain="buildmarket.app"` and force the browser to connect to production's FAPI (`clerk.buildmarket.app` returning `403 Forbidden: subdomain_not_allowed`), ensuring clean resolution to the staging instance (`clerk.staging.buildmarket.app`).
