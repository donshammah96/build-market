...
export const getProfessionalStoreUrl = (id: string) => `/professional-portal/stores/${id}`;

// Store Helpers
export const getStoreUrl = (id: string) => `/stores/${id}`;
export const getStoreByCategoryUrl = (category: string) => `/stores/${category.replace(/_/g, '-')}`;
export const getStoreByTypeUrl = (type: string) => `/stores/type/${type.replace(/_/g, '-')}`;
export const getStoreProductUrl = (storeId: string, productId: string) => `/stores/${storeId}/products/${productId}`;

// Professional Public Helpers
export const getProfessionalByCategoryUrl = (category: string) => `/professionals/${category.replace(/_/g, '-')}`;
export const getProfessionalReviewsUrl = (id: string) => `/professionals/${id}/reviews`;
export const getProfessionalPortfoliosUrl = (id: string) => `/professionals/${id}/portfolios`;
export const getProfessionalContactUrl = (id: string) => `/professionals/${id}/contact`;

// Professional Portal Detail Helpers
export const getProfessionalQuoteUrl = (id: string) => `/professional-portal/quotes/${id}`;
export const getProfessionalCertificateUrl = (id: string) => `/professional-portal/certificates/${id}`;
...
export const withQueryParams = (baseUrl: string, params: QueryParams): string => {
...
export const isActiveRoute = (currentPath: string, targetRoute: string, exact = false): boolean => {
...
export const getSearchUrl = (basePath: string, filters: SearchFilters): string => {
...
export const getPropertySearchUrl = (filters: SearchFilters & { type?: string; minPrice?: number; maxPrice?: number }): string => {
  return getSearchUrl(ROUTES.properties, filters as QueryParams);
};

// Pagination Helper
export const withPagination = (baseUrl: string, page: number, limit = 12): string => {
...
export const extractRouteParams = (path: string, pattern: string): Record<string, string> | null => {
  const pathParts = path.split('/').filter(Boolean);
  const patternParts = pattern.split('/').filter(Boolean);
...
export const isPublicRoute = (path: string): boolean => {
  const publicPaths = ['/', '/sign-in', '/sign-up', '/professionals', '/stores', '/properties'];
  return publicPaths.some((p) => path === p || path.startsWith(`${p}/`));
};

export const isAuthRoute = (path: string): boolean => {
  return path.startsWith('/sign-in') || path.startsWith('/sign-up') || path === '/auth-callback';
};
