/**
 * Professional Onboarding Wizard Components
 *
 * A multi-step wizard for professional onboarding with conditional steps
 * based on profession type (suppliers, real estate, etc.)
 */

// Step Components
export { default as ProfessionStep } from "./ProfessionStep";
export { default as DetailsStep } from "./DetailsStep";
export { default as StoreStep } from "./StoreStep";
export { default as PropertyStep } from "./PropertyStep";
export { default as CredentialsStep } from "./CredentialsStep";
export { default as DocumentsStep } from "./DocumentsStep";
export { default as ConsentStep } from "./ConsentStep";
export { default as ReviewStep } from "./ReviewStep";

// Types and utilities
export * from "./types";
export type { ConsentData, ConsentRecord } from "./ConsentStep";
