...
describe("reviewsService.submitProjectReview", () => {
  it("permits exactly one review by the completed project's client", async () => {
    vi.spyOn(reviewsRepository, "createEligibleProjectReview").mockResolvedValue({
      id: "review_1",
    } as any);
  ...
  it("returns the repository eligibility outcome without creating a review", async () => {
    vi.spyOn(reviewsRepository, "createEligibleProjectReview").mockResolvedValue(null);
    ...
    expect(result).toMatchObject({ ok: false, error: "review_not_eligible", status: 403 });
  });
});
