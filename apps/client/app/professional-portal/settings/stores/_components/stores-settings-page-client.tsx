...
export default function StoresSettingsPageClient() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [selectedStore, setSelectedStore] = useState<MyStoreWithStats | null>(
    null,
  );
  ...
  const handleDelete = (store: MyStoreWithStats) => {
                        ...
                        <div className="flex items-center gap-4 text-sm text-zinc-600">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className="font-semibold">
                              {currencyFormatter.format(store.totalRevenue)}
                            </span>
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="flex-1">
                            ...
                            {isVerified && (
                              <p className="text-sm text-emerald-600 mt-2">
                                Your store has been verified and is live on the
                                platform.
                              </p>
                            )}
                            {isPending && (
                              <p className="text-sm text-amber-600 mt-2">
                                Your store verification is under review. We will
                                notify you once complete.
                              </p>
                            )}
                            {isRejected && (
                              <p className="text-sm text-red-600 mt-2">
                                Please review the rejection reason above and
                                resubmit your store for verification.
                              </p>
                            )}
                            {verificationStatus === "UNVERIFIED" && (
                              <p className="text-sm text-zinc-600 mt-2">
                                Your store needs verification before publishing.
