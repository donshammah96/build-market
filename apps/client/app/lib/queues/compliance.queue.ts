...
import { AuditAction, IncidentSeverity, IncidentType, DataClass } from '@prisma/client';
...
export const userNotificationQueue = new Queue<UserNotificationJobData>('compliance-notifications', {
  connection: redisConnection as any,
  defaultJobOptions: {
    attempts: 3,
    backoff: {
...
export interface IncidentJobData {
  incidentId: string;
  type: 'EMERGENCY_PROTOCOL' | 'ODPC_NOTIFICATION' | 'USER_NOTIFICATION' | 'ESCALATION';
...
export interface UserNotificationJobData {
  incidentId: string;
  userIds: string[];
  template: 'BREACH_NOTIFICATION' | 'DATA_ACCESS_ALERT' | 'CONSENT_WITHDRAWAL_CONFIRMATION';
...
// Helper functions
export async function queueEmergencyProtocol(incidentId: string, severity: IncidentSeverity) {
