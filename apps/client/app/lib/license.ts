...
const baseLicenseSchema = z.object({
    expiryDate: z.date()
    .min(new Date(), { message: "License has already expired"}).optional(), // Optional because some legacy licenses might not show it
...
export const professionalLicenseSchema = z.discriminatedUnion("authority", [
    ...
    baseLicenseSchema.extend({
        ...
        licenseNumber: z.string().regex(REGEX_PATTERNS.EARB, { message: "Invalid EARB Licence Number. Format should be 'Reg. No. 12345' or '12345' " }),
    }),
    // --- CONTRACTORS ---
    baseLicenseSchema.extend({
        authority: z.literal("NCA"),
        licenseNumber: z.string().regex(REGEX_PATTERNS.NCA, { message: "Invalid NCA Licence Number. Format should be 'NCA/123/4567' or similar" }),
    }),
    // --- ENGINEERS ---
    baseLicenseSchema.extend({
        authority: z.literal("EBK"),
        licenseNumber: z.string().regex(REGEX_PATTERNS.EBK, { message: "Invalid EBK Licence Number. Format should be 'A1234' or 'B1234'" }),
    }),
    // --- ARCHITECTS & QS ---
    baseLicenseSchema.extend({
        authority: z.literal("BORAQS"),
        licenseNumber: z.string().regex(REGEX_PATTERNS.BORAQS, { message: "Invalid BORAQS Licence Number. Format should be 'A123' or 'QS123'" }),
    }),
    // --- ELECTRICIANS ---
    baseLicenseSchema.extend({
        authority: z.literal("ERC"),
        licenseNumber: z.string().regex(REGEX_PATTERNS.ERC, { message: "Invalid ERC Licence Number. Format should be 'Class A-1' or 'Class B-2'" }),
    }),
    // --- NEMA ---
    baseLicenseSchema.extend({
        authority: z.literal("NEMA"),
        licenseNumber: z.string().regex(REGEX_PATTERNS.NEMA, { message: "Invalid NEMA Licence Number. Format should be 'NEMA/EIA/5/1234' or 'NEMA/WM/1234'" }),
    }),
]);
