...
export function onboardingDomainErrorToClientMessage(code: string): string {
  switch (code) {
    case "not_found":
...
export function logOnboardingRouteOutcome(input: RouteLogInput): void {
  ...
  if (input.outcome === "internal_error" || input.httpStatus >= 500) {
    logger.error("Onboarding route outcome", undefined, payload);
  } else if (input.outcome === "success") {
    logger.info("Onboarding route outcome", payload);
  } else {
    logger.warn("Onboarding route outcome", payload);
  }
}
