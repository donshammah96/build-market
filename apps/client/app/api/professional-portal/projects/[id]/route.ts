...
export const GET = withAuth<{ id: string }>(async (req: NextRequest, { dbUserId }, params) => {
  ...
  logger.info('Fetching project', { correlationId, projectId: id, userId: dbUserId });
  ...
  return executeResilient(
    async () => {
      ...
      if (!project) {
        logger.warn('Project not found', { correlationId, projectId: id, userId: dbUserId });
        return apiError("Project not found", HttpStatus.NOT_FOUND);
      }

      logger.info('Project fetched successfully', { correlationId, projectId: id });
      return project;
    },
    {
...
export const PATCH = withAuth<{ id: string }>(async (req: NextRequest, { dbUserId }, params) => {
  ...
  if (!validation.success) {
    logger.warn('Project update validation failed', { correlationId, projectId: id, errors: validation.error.issues });
  ...
  logger.info('Updating project', { correlationId, projectId: id, userId: dbUserId });
  ...
  return executeResilient(
    async () => {
      ...
      if (!existingProject) {
        logger.warn('Project not found for update', { correlationId, projectId: id, userId: dbUserId });
        return apiError("Project not found", HttpStatus.NOT_FOUND);
      }
      ...
      logger.info('Project updated successfully', { correlationId, projectId: id });
...
export const DELETE = withAuth<{ id: string }>(async (req: NextRequest, { dbUserId }, params) => {
  ...
  logger.info('Deleting project', { correlationId, projectId: id, userId: dbUserId });
  ...
  return executeResilient(
    async () => {
      ...
      if (!existingProject) {
        logger.warn('Project not found for deletion', { correlationId, projectId: id, userId: dbUserId });
        return apiError("Project not found", HttpStatus.NOT_FOUND);
      }
      ...
      logger.info('Project deleted successfully', { correlationId, projectId: id });
