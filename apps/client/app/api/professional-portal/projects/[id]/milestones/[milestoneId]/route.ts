  ...
  async (req: NextRequest, { dbUserId }, params): Promise<NextResponse> => {
    ...
    const result = await resilientExecutor.execute(
      async () => {
        ...
        if (!milestone) return { error: "not_found" as const };
    ...
    const milestone = result.data?.data;
    const response = apiSuccess(milestone, HttpStatus.OK);
    const version = (milestone as { version?: number })?.version ?? 0;
  ...
  async (req: NextRequest, { dbUserId }, params): Promise<NextResponse> => {
    ...
    try {
      ...
      if (result.success && result.data) {
        const responseData = {
          ...(result.data as { milestone: Record<string, unknown> }).milestone,
