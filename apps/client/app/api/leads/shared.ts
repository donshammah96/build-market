...
export function leadDomainErrorToClientMessage(code: string): string {
  switch (code) {
    case "not_found":
...
export function logLeadsRouteOutcome(input: RouteLogInput): void {
  ...
  if (input.outcome === "internal_error" || input.httpStatus >= 500) {
    logger.error("Leads route outcome", undefined, payload);
  } else if (input.outcome === "success") {
    logger.info("Leads route outcome", payload);
  } else {
    logger.warn("Leads route outcome", payload);
  }
}
