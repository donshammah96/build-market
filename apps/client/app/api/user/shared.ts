...
export function userDomainErrorToClientMessage(code: string): string {
  switch (code) {
    case "not_found":
      return "User resource not found";
    case "forbidden":
      return "Forbidden";
    case "conflict":
...
export function logUserRouteOutcome(input: RouteLogInput): void {
  ...
  if (input.outcome === "internal_error" || input.httpStatus >= 500) {
    logger.error("User route outcome", undefined, payload);
  } else if (input.outcome === "success") {
    logger.info("User route outcome", payload);
  } else {
    logger.warn("User route outcome", payload);
  }
}
