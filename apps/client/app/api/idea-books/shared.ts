...
export type IdeaBooksRouteOutcome =
  | "success"
  | "domain_error"
  | "validation_error"
  | "rate_limited"
...
export function ideaBookDomainErrorToClientMessage(code: string): string {
  switch (code) {
    case "not_found":
      return "Idea book not found";
    case "forbidden":
      return "Forbidden";
    case "asset_not_found":
    ...
    default:
      return "Idea book operation failed";
  }
}

export function logIdeaBooksRouteOutcome(input: RouteLogInput): void {
  ...
  const payload = {
    ...
    resourceType: "idea-book",
    ...(input.domainError ? { domainError: input.domainError } : {}),
    ...(input.resourceId ? { resourceId: input.resourceId } : {}),
  };

  if (input.outcome === "internal_error" || input.httpStatus >= 500) {
    logger.error("Idea-books route outcome", undefined, payload);
  } else if (input.outcome === "success") {
    logger.info("Idea-books route outcome", payload);
  } else {
    logger.warn("Idea-books route outcome", payload);
  }
}
