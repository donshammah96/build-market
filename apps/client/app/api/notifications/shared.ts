...
export type NotificationsRouteOutcome =
  | "success"
  | "domain_error"
  | "validation_error"
  | "rate_limited"
...
type RouteLogInput = {
  ...
  outcome: NotificationsRouteOutcome;
  httpStatus: number;
  durationMs: number;
  domainError?: string;
  resourceId?: string;
...
export function notificationDomainErrorToClientMessage(code: string): string {
  switch (code) {
    case "not_found":
...
export function logNotificationsRouteOutcome(input: RouteLogInput): void {
  ...
  if (input.outcome === "internal_error" || input.httpStatus >= 500) {
    logger.error("Notifications route outcome", undefined, payload);
  } else if (input.outcome === "success") {
    logger.info("Notifications route outcome", payload);
  } else {
    logger.warn("Notifications route outcome", payload);
  }
}
