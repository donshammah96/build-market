...
const OPERATION_NAME = "reconcile_idempotency_key";
...
function mapErrorMessage(error: string): string {
  switch (error) {
    ...
    case "invalid_state":
      return "Invalid idempotency state";
    case "conflict":
      return "Mutation already completed";
    default:
      return "Unable to reconcile idempotency key";
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const result = await onboardingRemediationService.reconcileIdempotencyKey(
    actor,
    key,
  );
