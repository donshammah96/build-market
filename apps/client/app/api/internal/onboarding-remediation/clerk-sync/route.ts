...
const ROUTE_PATTERN = "/api/internal/onboarding-remediation/clerk-sync";
const OPERATION_NAME = "sync_clerk_metadata";
...
function mapErrorMessage(error: string): string {
  switch (error) {
    ...
    case "clerk_sync_failed":
      return "Unable to sync Clerk metadata";
    default:
      return "Unable to sync Clerk metadata";
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const result = await onboardingRemediationService.syncClerkMetadata(
    actor,
    userId,
  );
