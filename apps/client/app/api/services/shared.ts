...
export type ServicesRouteOutcome =
  | "success"
  | "domain_error"
  | "validation_error"
  | "rate_limited"
...
export function serviceDomainErrorToClientMessage(code: string): string {
  switch (code) {
    case "not_found":
...
export function logServicesRouteOutcome(input: RouteLogInput): void {
  ...
  if (input.outcome === "internal_error" || input.httpStatus >= 500) {
    logger.error("Services route outcome", undefined, payload);
  } else if (input.outcome === "success") {
    logger.info("Services route outcome", payload);
  } else {
    logger.warn("Services route outcome", payload);
  }
}
