...
export type MessagingRouteOutcome =
  | "success"
  | "domain_error"
  | "validation_error"
  | "rate_limited"
...
export function messagingDomainErrorToClientMessage(code: string): string {
  switch (code) {
    case "not_found":
...
export function logMessagingRouteOutcome(input: RouteLogInput): void {
  ...
  if (input.outcome === "internal_error" || input.httpStatus >= 500) {
    logger.error("Messaging route outcome", undefined, payload);
  } else if (input.outcome === "success") {
    logger.info("Messaging route outcome", payload);
  } else {
    logger.warn("Messaging route outcome", payload);
  }
}
