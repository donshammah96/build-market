...
export default function CompleteClientProfilePage() {
  ...
  const calculateLocalCompletion = () => {
    ...
    const filledRequired = requiredFields.filter(f => 
      formData[f as keyof FormData]?.trim().length > 0
    ).length;
    const filledOptional = optionalFields.filter(f => 
      formData[f as keyof FormData]?.trim().length > 0
    ).length;
    
    const requiredPercentage = Math.round((filledRequired / requiredFields.length) * 80);
    const optionalPercentage = Math.round((filledOptional / optionalFields.length) * 20);
      ...
      <main className="container mx-auto px-4 md:px-8 py-8 pt-24 max-w-3xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-500 mt-2">
            Fill in the required information to unlock all features and get personalized recommendations.
          </p>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-500">
                    {localPercentage >= 80 ? 'Almost there!' : 'Complete required fields to continue'}
                  </p>
                </div>
              </div>
          ...
          <Card className="border-zinc-200 shadow-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* County */}
                <div>
                  <Label htmlFor="county" className="text-sm font-medium text-zinc-700">
                    County
                  </Label>
                  <Input
                    id="county"
