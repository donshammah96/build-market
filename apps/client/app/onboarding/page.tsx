            ...
            <div className="w-full max-w-5xl relative z-10">
                ...
                >
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div className={cn("w-12 h-0.5 transition-colors duration-500", step >= 2 ? "bg-emerald-500" : "bg-zinc-800")} />
                        <StepIndicator current={step} stepNumber={2} label="Details" />
                    </div>
                    
                    <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight mb-3">
                        {step === 1 ? "Build your legacy." : role === 'client' ? "Tell us about your dream." : "Showcase your expertise."}
                    </h1>
                                    ...
                                    {submitting && (
                                        <div className="absolute inset-0 bg-zinc-950/80 backdrop-blur-sm z-50 flex items-center justify-center rounded-xl">
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <span className="text-emerald-500 font-medium animate-pulse">Creating Profile...</span>
                                            </div>
                                        </div>
                                    )}
    ...
    return (
        <div className="flex flex-col items-center gap-2">
            <div className={cn(
                ...
                isActive ? "bg-emerald-500 border-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.5)]" : "bg-transparent border-zinc-700 text-zinc-500"
            )}>
                {isActive ? <CheckCircle2 size={14} /> : stepNumber}
            </div>
            <span className={cn("text-xs font-medium uppercase tracking-wider", isActive ? "text-emerald-400" : "text-zinc-600")}>
                {label}
            </span>
        </div>
    );
...
const RoleCard = ({ icon, title, description, onClick, delay, highlight }: any) => (
    ...
    >
        <div className={cn(
            ...
            highlight ? "bg-emerald-500/10 text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white" : "bg-white/5 text-zinc-400 group-hover:bg-white group-hover:text-zinc-900"
        )}>
            {icon}
        </div>
