...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      }): Chainable<{ transactionId: string; checkoutRequestId: string; merchantRequestId: string }>;
...
Cypress.Commands.add("initStagingRun", (scenario: string, actorLabel = "cypress-e2e") => {
  ...
  return cy.task("stagingTestControl:createRun", { scenario, actorLabel }).then((res: any) => {
    if (!res || !res.runId) {
      throw new Error("Failed to initialize staging test run via node task");
    }
    return res;
  });
});
...
Cypress.Commands.add("loginStagingUser", (role: "CLIENT" | "PROFESSIONAL") => {
  return cy.task("stagingTestControl:issueSession", { role }).then((res: any) => {
...
Cypress.Commands.add("seedStagingScenario", (scenario, payload = {}) => {
  return cy.task("stagingTestControl:seedScenario", { scenario, payload }).then((res: any) => {
