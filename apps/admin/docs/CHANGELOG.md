...
- **TypeScript Project References Build Failure (`TS6305` & `TS2307`):** Resolved CI/Vercel compile errors where imports of `@build/redis` inside `packages/resilience` and `apps/admin` caused typescript project references validation to crash. Resolved by adding a `"build": "tsc"` script to `packages/redis/package.json` and routing its entry points (`main`, `types`, and `exports`) to resolve to its built `./dist` folder. To prevent CommonJS-to-ESM resolution mismatches under `NodeNext` resolution, removed `"type": "module"` from `@build/redis` (aligning it with other monorepo packages as a dual-compliant CommonJS library). Also added the missing `@build/redis` project reference to [`packages/resilience/tsconfig.json`](file:///c:/Users/User/build-market/packages/resilience/tsconfig.json).
