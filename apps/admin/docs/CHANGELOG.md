...
- **Admin Sign-In Redirect Loop (RC-4 — Satellite Middleware Redirect):** Updated `middleware.ts` to manually redirect unauthenticated requests directly to the primary domain's sign-in URL when running in Clerk satellite mode (`NEXT_PUBLIC_CLERK_IS_SATELLITE=true`), preventing the default `redirectToSignIn()` from sending users to the local relative `/sign-in` route (which caused unauthorized redirect loops in production).
