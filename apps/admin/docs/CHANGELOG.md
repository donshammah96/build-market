...
- **TypeScript Project References Build Failure (`TS6305`)**: Resolved CI/Vercel compile errors where imports of `@build/redis` inside the system health service caused typescript project references validation to crash on a missing `/packages/redis/dist/index.d.ts` output file. Fixed by adding a `"build": "tsc"` script to `packages/redis/package.json` and updating its `main`, `types`, and `exports` mapping to resolve to its built `./dist` folders (aligning it with other monorepo workspaces).
