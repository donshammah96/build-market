...
const envBoundaryPath = path.join(appRoot, "src/lib/infrastructure/env.ts");
...
function collectBoundaryKeys() {
  ...
  const keys = new Set();
  for (const match of content.matchAll(/^\s{2}([A-Z0-9_]+):/gm)) {
    keys.add(match[1]);
  }
  return keys;
}
