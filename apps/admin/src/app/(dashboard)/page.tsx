...
export default async function DashboardPage() {
  ...
  const verificationRate = stats.professionalCount > 0 
    ? ((stats.verifiedProfessionalCount / stats.professionalCount) * 100).toFixed(1) 
    : "0.0";
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-end justify-between border-b border-zinc-200 pb-6">
        <div>
          ...
          <p className="text-zinc-500 mt-1">System-wide metrics and performance indicators.</p>
        </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-zinc-50 rounded-lg border border-zinc-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-zinc-900">Database Status</p>
                    <p className="text-xs text-zinc-500">PostgreSQL (Neon)</p>
                  </div>
                </div>
                <div className="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-1 rounded">Healthy</div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-zinc-50 rounded-lg border border-zinc-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-zinc-900">Auth Service</p>
                    <p className="text-xs text-zinc-500">Clerk</p>
                  </div>
                </div>
                <div className="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-1 rounded">Online</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="col-span-1 border-zinc-200 shadow-sm">
          ...
          <CardContent>
            <div className="text-center py-8">
              <p className="text-zinc-500 text-sm">No critical alerts.</p>
              <p className="text-zinc-400 text-xs mt-1">All systems are running smoothly.</p>
            </div>
          </CardContent>
        </Card>
...
function MetricCard({ title, value, icon: Icon, trend, subValue, color }: MetricCardProps) {
