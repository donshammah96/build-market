    ...
    <div className="space-y-6">
       <div className="flex items-center gap-4">
            ...
            <div className="ml-auto">
                 <span className="px-3 py-1 bg-secondary rounded-full text-sm capitalize">{user.role}</span>
            </div>
        </div>
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                     <div className="flex justify-between py-2 border-b">
                        ...
                        <span className="text-sm text-muted-foreground break-all">{user.id}</span>
                     </div>
                      <div className="flex justify-between py-2 border-b">
                        <span className="font-medium">Clerk ID</span>
                        <span className="text-sm text-muted-foreground break-all">{user.clerkId}</span>
                     </div>
                     <div className="flex justify-between py-2 border-b">
                     ...
                     {user.clientProfile && (
                         <div className="pt-4">
                             <h4 className="font-semibold mb-2">Client Details</h4>
                              <p className="text-sm text-muted-foreground">{user.clientProfile.city}, {user.clientProfile.county}</p>
                         </div>
                     )}
                     ...
                     {user.professionalProfile && (
                         <div className="pt-4">
                              ...
                              <Link href={`/professionals/${user.id}`} className="text-blue-600 hover:underline text-sm">
                                View Professional Profile ({user.professionalProfile.companyName})
                              </Link>
                         </div>
                     )}
                </CardContent>
                    ...
                    {user.orders.length > 0 ? (
                        <ul className="space-y-2">
                            {user.orders.map(order => (
                                <li key={order.id} className="text-sm flex justify-between p-2 bg-secondary/50 rounded">
