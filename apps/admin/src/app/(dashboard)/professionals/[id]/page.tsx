...
import { CheckCircle, XCircle, FileText, Globe, MapPin, User as UserIcon } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
            ...
            <div>
                 <h1 className="text-3xl font-bold tracking-tight">{pro.companyName}</h1>
                 <p className="text-muted-foreground flex items-center gap-2">
                    <UserIcon className="h-4 w-4" /> {pro.user.firstName} {pro.user.lastName} | {pro.user.email}
                 </p>
            </div>
        </div>
        
        <div className="flex items-center gap-2">
            <div className={`px-3 py-1 rounded-full text-sm font-medium flex items-center gap-2 ${pro.verified ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                {pro.verified ? <CheckCircle className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}
            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                        ...
                        <div>
                         ...
                         <div>
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <p>{pro.city}, {pro.county}, {pro.country}</p>
                            </div>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Website</p>
                            {pro.website ? (
                                <Link href={pro.website} target="_blank" className="text-blue-600 hover:underline flex items-center gap-1">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">{pro.bio || "No bio provided."}</p>
                    </div>
                     <div>
                        <p className="text-sm font-medium text-muted-foreground mb-1">Services</p>
                        <div className="flex flex-wrap gap-2">
                            {pro.servicesOffered.map((s) => (
                                <span key={s} className="px-2 py-1 bg-secondary text-secondary-foreground rounded text-xs">
                                    {s}
                                </span>
                            ))}
                        </div>
                ...
                <CardContent>
                    {pro.certificates.length > 0 ? (
                        <div className="space-y-4">
                                ...
                                <div key={cert.id} className="flex items-center justify-between p-3 border rounded-lg">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">{cert.issuer} - Exp: {cert.expiryDate ? new Date(cert.expiryDate).toLocaleDateString() : "N/A"}</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <Button variant="outline" size="sm" asChild>
                                            <Link href={cert.fileUrl} target="_blank">View</Link>
                                        </Button>
                                    </div>
                                </div>
                            ))}
                            ...
                            <h4 className="text-sm font-medium mb-2">Manage Certificates</h4>
                            <CertificateManager certificates={pro.certificates.map(c => ({ id: c.id, name: c.name, fileUrl: c.fileUrl }))} />
