  ...
  return executeResilient(
    async () => {
      ...
      const [
        ...
        prisma.professionalLicense.count({ where: whereClause }),
        prisma.professionalLicense.count({
          where: { ...whereClause, status: "VERIFIED" },
        }),
        prisma.propertyAttachment.count({ where: whereClause }),
        prisma.propertyAttachment.count({
          where: { ...whereClause, uploadedById: dbUserId },
        }),
      ]);
      ...
      const stats = {
        overview: {
          ...
          avgVerificationTimeHours:
            Math.round(avgVerificationTimeHours * 10) / 10,
        },
        professionals: {
