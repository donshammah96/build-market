...
// Helper getters to read from env dynamically, avoiding ESM caching issues in tests
export function getCurrentKeyVersion(): string {
  return adminEnvConfig.CURRENT_KEY_VERSION || "v1";
}

export function getLegacyFormatDeadline(): Date {
  return adminEnvConfig.LEGACY_FORMAT_DEADLINE
    ? new Date(adminEnvConfig.LEGACY_FORMAT_DEADLINE)
    : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000); // 90 days from now
}

export function isEncryptionMigrationMode(): boolean {
  return adminEnvConfig.ENCRYPTION_MIGRATION_MODE === true;
}
 ...
 */
export function loadEncryptionKeys(forceReload = false): void {
  if (keysLoaded && !forceReload) {
    return;
  }

  // Clear existing keys first
  for (const key of Object.keys(ENCRYPTION_KEYS)) {
    delete ENCRYPTION_KEYS[key];
  }
  ...
  if (!ENCRYPTION_KEYS[currentVersion]) {
    throw new Error(
      `FATAL: CURRENT_KEY_VERSION is set to "${currentVersion}" ` +
        `but ENCRYPTION_KEY_${currentVersion.toUpperCase()} is not configured.`,
    );
  }
...
try {
  ...
  // In test environment, we might not have environment keys set at import time.
  // We swallow this error to allow lazy loading/testing. In production, we fail fast.
  if (adminEnvConfig.NODE_ENV !== "test") {
    throw error;
  }
}
...
function getCurrentKey(): Buffer {
  loadEncryptionKeys();
  return ENCRYPTION_KEYS[getCurrentKeyVersion()]!;
}
...
export class FieldEncryption {
  ...
  static encryptDeterministic(text: string): string {
    ...
    try {
      ...
      return `${currentVersion}:${iv.toString("hex")}:${authTag.toString("hex")}:${encrypted}`;
    } catch (error) {
      logger.error(
        "Encryption failed",
  ...
  static decrypt(encryptedText: string): string {
    ...
    try {
      ...
      const legacyDeadline = getLegacyFormatDeadline();
      ...
      if (parts.length === 4 && parts[0]?.startsWith("v")) {
        ...
        if (new Date() > legacyDeadline) {
          ...
          logger.warn(
            `Legacy encryption format detected. Support ends on ${legacyDeadline.toISOString()}. Run key rotation migration script.`,
          );
        }
      } else {
  ...
  static reEncrypt(
    ...
    if (encryptedText.startsWith(`${currentVersion}:`)) {
  ...
  static validateConfiguration(): {
    ...
    const currentVersion = getCurrentKeyVersion();
    const legacyDeadline = getLegacyFormatDeadline();
    const migrationMode = isEncryptionMigrationMode();
    ...
    if (!ENCRYPTION_KEYS[currentVersion]) {
      errors.push(`Current key version ${currentVersion} not found`);
    }
  ...
  static getMetadata(): {
    ...
    return {
      ...
      legacyDeadline: getLegacyFormatDeadline().toISOString(),
      migrationMode: isEncryptionMigrationMode(),
    };
  }
}
