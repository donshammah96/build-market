...
export const adminBaseEnvSchema = z.object({
  ...
  NEXT_PUBLIC_VERIFICATION_OPS_URL: optionalUrl,
  ...
  SETTLED_ARCHIVAL_CRON: z.string().min(1).optional(),
  SETTLED_ARCHIVAL_BATCH_SIZE: z.coerce.number().int().positive().optional(),
  NATS_URL: z.string().min(1).optional(),
  OTEL_EXPORTER_OTLP_ENDPOINT: z.string().min(1).optional(),
  OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: z.string().min(1).optional(),
  ...
  .refine(
    (data) => {
      if (
        ...
        return Boolean(
          data.CLERK_SECRET_KEY && data.CLERK_SECRET_KEY.length > 0,
        );
      }
      return true;
    },
    {
      message:
        "CLERK_SECRET_KEY is required when ADMIN_DEPLOYMENT_PROFILE is production or staging",
  ...
  .refine(
    (data) => {
      if (
        ...
        return Boolean(
          data.INTERNAL_API_SECRET && data.INTERNAL_API_SECRET.length > 0,
        );
      }
      return true;
    },
    {
      message:
        "INTERNAL_API_SECRET is required when ADMIN_DEPLOYMENT_PROFILE is production or staging",
...
export function isStaticBuildPhase(): boolean {
  return (
    process.env.NEXT_PHASE === "phase-production-build" || // bootstrap-only: build phase detection
    process.env.npm_lifecycle_event === "build" // bootstrap-only: build lifecycle detection
  );
}
...
export function validateAdminEnv(): AdminEnvConfig {
  const cleanedEnv = cleanEnv(process.env); // bootstrap-only exception
