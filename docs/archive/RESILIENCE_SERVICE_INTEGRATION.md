...
import { resilientFetch } from "@build/resilience";
...
await executor.execute(async () => prisma.professional.findMany(), {
  criticality: "normal",
...
await executor.execute(async () => stripe.charges.create(chargeData), {
...
await executor.execute(async () => analytics.track(event), {
...
await executor.execute(async () => processPayment(paymentData), {
  criticality: "critical",
...
await executor.execute(async () => searchIndex.search(query), {
  criticality: "normal",
...
await executor.execute(async () => sendEmail(recipient, message), {
  criticality: "background",
...
1. ✅ Add `@build/resilience` dependency
