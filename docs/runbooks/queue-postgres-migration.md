...
>
> Per [adr-bullmq-nats-queue-split.md](../../apps/workers/docs/adr/adr-bullmq-nats-queue-split.md#L40), PostgreSQL queue runner migration is deferred to BullMQ v6's `IQueueBackend` abstraction. Until BullMQ v6 is adopted, all queues must remain on `QUEUE_BACKEND=redis`.
